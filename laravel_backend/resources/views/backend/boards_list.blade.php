...
<div class="page-content">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-3 text-end">
                                                <a href="{{route('backend.boards.create') }}" title="Add Record" class="btn btn-info btn-md" ><i class="fa fa-plus"></i> Add Record</a>
                                        ...
                                        <table class="table table-bordered datatable" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                            ...
                                            <tr class="bg-info text-white">    
                                                ...
                                                <th>ID</th><th>WORKSPACE</th><th>TITLE</th><th>VISIBILITY</th><th>ACTIVE</th>
                                            ...
                                            @foreach($rows as $row)
                                            <tr>
                                                <td> {{ $row->id }}</td><td>{{getfieldval("$row->workspace_id","workspaces","title")}}</td><td> {{ $row->title }}</td><td>{{getfieldval("$row->visibility","active_status","is_active")}}</td><td>{{getfieldval("$row->active","active_status","is_active")}}</td>
                                                <td class="text-center"> 
                                                <div class="btn-group">
                                                    <a href="{{url('backend/boards/'.id2str($row->id).'/edit')}}" class="btn btn-info" ><i class="fas fa-edit"></i></a>
                                                    &nbsp;&nbsp;&nbsp;
                                                     <form action="{{ route('backend.boards.delete')}}" method="post">
                                                        @csrf
                    ...
                    {{-- <div class="modal fade bs-example-modal-center " tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
                        ...
                        <form method="post" action="{{route('backend.boards.search')}}" >
                                        @csrf
